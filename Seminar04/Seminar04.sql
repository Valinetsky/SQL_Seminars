...
*/
DROP DATABASE IF EXISTS seminar04;
CREATE DATABASE IF NOT EXISTS seminar04;
...
-- 2.2 Выборка через JOIN с подзапросом
SELECT shops.*, katze.* -- Тут больше полей, чем в запросах 2.1 и 2.3
-- Можно ставить просто 
-- SELECT shops.*
-- и вывод будет аналогичен двум другим способам
FROM shops
JOIN
(SELECT cats.name, shops_id
FROM cats WHERE cats.name = "Murzik") AS katze
ON shops.id = katze.shops_id;

-- 2.3 Выборка через WHERE
SELECT *
FROM shops
WHERE id IN (
	SELECT shops_id
...
SELECT analysis.an_name, analysis.an_cost -- Чистый вывод по двум параметрам
-- SELECT analysis.an_name, analysis.an_cost, orders.ord_datetime - Вывод с таймингами
FROM analysis
INNER JOIN orders
ON analysis.an_id = orders.ord_an 
AND DATE(orders.ord_datetime) 
BETWEEN @date 
AND DATE_ADD(@date, interval 1 WEEK);

/*
Для самопроверки:
в таблице Orders заказы есть только в первых трех позициях 
('Общий анализ крови', 'Биохимия крови' и 'Анализ крови на глюкозу'),
но если поправить строку (произвольный выбор) - 169
с
('2020-02-11 07:05:33', 1),
на
('2020-02-11 07:05:33', 7),
то последней строкой вывода запроса будет:
-----------------------
Тест на COVID-19	160
-----------------------
Работает! Ура!!!
*/